...
const e = (target, ...sources) => extend$2(target, ...sources);
...
function hasCallback(args) {
  if (isPlainObject$2(args) && [API_SUCCESS, API_FAIL, API_COMPLETE].find((cb) => isFunction$2(args[cb]))) {
    return true;
  }
  return false;
}
...
function initWrapper(protocols2) {
  ...
  return function wrapper(methodName, method) {
    ...
    const needWrapper = hasProtocol || isFunction$2(protocols2.returnValue) || isContextApi(methodName) || isTaskApi(methodName);
    const hasMethod = hasProtocol || isFunction$2(method);
...
function initRuntimeSocketService() {
  const hosts = "100.122.53.103,10.135.138.180,127.0.0.1";
  const port = "8090";
  const id = "mp-weixin_iDfoea";
...
utils$4.inherits(AxiosError$2, Error, {
...
isFetchSupported && ((res) => {
  ["text", "arrayBuffer", "blob", "formData", "stream"].forEach((type) => {
    !resolvers[type] && (resolvers[type] = utils$4.isFunction(res[type]) ? (res2) => res2[type]() : (_, config) => {
...
validators$1.spelling = function spelling(correctSpelling) {
  return (value, opt) => {
    index.__f__("warn", "at node_modules/axios/lib/helpers/validator.js:58", `${opt} is likely a misspelling of ${correctSpelling}`);
    return true;
  };
};
...
var format$1 = function format(config, resolve2, reject) {
  ...
  const uniConfig = {
    ...config,
    url: buildURL2(fullPath, config.params, config.paramsSerializer),
    // uniapp 用的是 header
  ...
  uniConfig.complete = function(response) {
    ...
    settle2(resolve2, reject, result);
  };
  return uniConfig;
};
...
function uniappAdapter(config = {}) {
  return new Promise(function dispatchUniApp(resolve2, reject) {
    ...
    if (isUploadFile2(config)) {
      requestTask = index.uploadFile(uniConfig);
    } else {
      requestTask = index.request(uniConfig);
    }
  });
}
axiosAdapterUniapp.exports = uniappAdapter;
axiosAdapterUniapp.exports.default = uniappAdapter;
var axiosAdapterUniappExports = axiosAdapterUniapp.exports;
const uniappAdapter$1 = /* @__PURE__ */ getDefaultExportFromCjs(axiosAdapterUniappExports);
...
exports.sr = sr;
exports.t = t;
exports.uniappAdapter = uniappAdapter$1;
exports.watch = watch;
exports.wx$1 = wx$1;
